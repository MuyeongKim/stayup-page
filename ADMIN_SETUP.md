...
관리 화면은 `https://www.stayup-ai.com/admin/`에서 열립니다. GitHub 쓰기 권한이 있는 계정으로 로그인한 뒤 Stay-Up과 FireHawks 활동을 추가·수정·숨김 처리하고, FireHawks의 다음 출전 일정도 관리할 수 있습니다.
...
FireHawks 출전 일정은 FireHawks 탭 상단의 **FireHawks 출전 일정 관리 → 새 일정 등록**에서 입력합니다. 여러 일정을 미리 등록해 둘 수 있으며, 공개 페이지에는 진행 중인 일정 또는 가장 가까운 예정 일정 한 건이 자동으로 표시됩니다. 종료된 일정은 관리 화면에 보존되고, 숨김 처리한 일정은 공개 페이지에 표시되지 않습니다.

관리 화면의 GitHub 토큰은 `sessionStorage`에만 보관되어 탭을 닫으면 브라우저에서 삭제됩니다. 공용 기기에서는 반드시 로그아웃하고, 기기 노출이 의심되면 GitHub **Settings → Applications → Authorized OAuth Apps**에서도 이 OAuth App 권한을 철회합니다. 저장 직전에 `master` 브랜치가 다른 사용자에 의해 변경되었는지 다시 확인하고, 변경이 있으면 덮어쓰지 않고 새로고침을 요청합니다. 활동이나 일정을 숨겨도 JSON 기록과 사진은 삭제되지 않으며 언제든 다시 공개할 수 있습니다. 현재 저장소는 공개 상태이므로 숨김은 사이트 표시만 제어할 뿐 보안상 비공개를 의미하지 않습니다. 민감한 초안이나 사진은 등록하지 않습니다.
